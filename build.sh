...
rm -f infinity-guard-1.2.0.jar
# Explicit, sorted entry list -> deterministic order regardless of FS listing.
jar --create \
    --file infinity-guard-1.2.0.jar \
    --manifest src/META-INF/MANIFEST.MF \
    --date "$EPOCH" \
    -C classes 'infinity/GuardAgent$HttpResult.class' \
    -C classes 'infinity/GuardAgent.class'

echo "Built infinity-guard-1.2.0.jar"
sha1sum infinity-guard-1.2.0.jar 2>/dev/null || shasum -a 1 infinity-guard-1.2.0.jar
